...
public:
    ...
    WriteAccess operator[](const K &key) {
        size_t bucket_key = hasher(key) % BUCKET_COUNT;
        auto &current_bucket = buckets[bucket_key];
        return WriteAccess{lock_guard(current_bucket.bucket_mutex), current_bucket.bucket_map[key]};
    }

    ReadAccess At(const K &key) const {
        size_t bucket_key = hasher(key) % BUCKET_COUNT;
        auto &current_bucket = buckets[bucket_key];
        return ReadAccess{lock_guard(current_bucket.bucket_mutex), current_bucket.bucket_map.at(key)};
    }

    bool Has(const K &key) const {
        size_t bucket_key = hasher(key) % BUCKET_COUNT;
        return buckets[bucket_key].bucket_map.count(key);
    }
