...
void TestSplit() {
    string s = "What.a.day.to.be.alive";
    vector<string_view> expected = {"What", "a", "day", "to", "be", "alive"};
...
void TestReverse() {
    ...
    vector<string_view> expected = {"What", "a", "day", "to", "be", "alive"};
    Domain domain(s);
    Range r = domain.GetReversedParts();
    reverse(begin(expected), end(expected));
    ASSERT_EQUAL(vector<string_view>(r.begin(), r.end()), expected);
}

void TestSelfSubdomain() {
    string s = "What.a.day.to.be.alive";
    Domain domain(s);
    ASSERT(IsSubdomain((domain), Domain(s)));
};

void TestWrongArgs() {
    ASSERT(!IsSubdomain(Domain("ru"), Domain("com.ru")));
}

void TestNoAbsortion() {
    vector<Domain> domains = {Domain("ru"), Domain("com.ru"), Domain("by.com.ru")};
    DomainChecker checker(begin(domains), end(domains));
...
void TestWrongOutput() {
    stringstream out;
    PrintCheckResults(vector<bool>({true, false}), out);
...
void TestBadGetline() {
    ...
    ASSERT_EQUAL(domains, vector<Domain>({Domain("by"), Domain("ru"), Domain("com"), Domain("ua.en.org")}));
}
int main() {
    TestRunner tr;
    ...
    RUN_TEST(tr, TestWrongArgs);
    RUN_TEST(tr, TestNoAbsortion);
    RUN_TEST(tr, TestWrongBool);
    RUN_TEST(tr, TestWrongOutput);
    RUN_TEST(tr, TestBadGetline);
    return 0;
}