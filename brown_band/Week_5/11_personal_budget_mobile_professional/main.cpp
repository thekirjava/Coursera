...
public:
    ...
    void CombineWith(const BulkLinearUpdater &other) {
        ...
        add_.delta_earned = add_.delta_earned * other.tax_.factor + other.add_.delta_earned;
        add_.delta_spent += other.add_.delta_spent;
    }

    MoneyBalance Collapse(MoneyBalance origin, IndexSegment segment) const {
        return {origin.earned * tax_.factor + add_.delta_earned * segment.length(),
...
private:
    ...
    public:
        ...
        Data ProcessPartial(const NodeHolder &, IndexSegment, const Data &left_result, const Data &right_result) const {
            return {left_result.earned + right_result.earned, left_result.spent + right_result.spent};
        }
    };
    ...
    public:
        ...
        void ProcessPartial(const NodeHolder &node, IndexSegment) const {
            node->data = {(node->left ? node->left->data.earned : Data().earned) +
                          ...
                          (node->left ? node->left->data.spent : Data().spent) +
                          (node->right ? node->right->data.spent : Data().spent)};
        }
...
class BudgetManager : public SummingSegmentTree<MoneyBalance, BulkLinearUpdater> {
public:
...
struct SpendRequest : ModifyRequest {
    ...
    void Process(BudgetManager &manager) const override {
        ...
        manager.AddBulkOperation(date_segment, BulkMoneyAdder{0, daily_outcome});
    }
