...
array<MoneyBalance, VERTEX_COUNT> tree_values, tree_add;
...
void Push(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        if (w < VERTEX_COUNT) {
            ...
            (tree_add[w].income *= tree_factor[v]) += tree_add[v].income;
            tree_add[w].outcome += tree_add[v].outcome;
            (tree_values[w].income *= tree_factor[v]) += tree_add[v].income * (r - l) / 2;
            tree_values[w].outcome += tree_add[v].outcome * (r - l) / 2;
        }
    }
    tree_factor[v] = 1;
    tree_add[v] = {0, 0};
...
MoneyBalance ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
    ...
    MoneyBalance lhs = ComputeSum(v * 2, l, (l + r) / 2, ql, qr);
    MoneyBalance rhs = ComputeSum(v * 2 + 1, (l + r) / 2, r, ql, qr);
    return {lhs.income + rhs.income, lhs.outcome + rhs.outcome};
}

void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, MoneyBalance value) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return;
    }
    Push(v, l, r);
    if (ql <= l && r <= qr) {
        tree_add[v].income += value.income;
        tree_add[v].outcome += value.outcome;
        tree_values[v].income += value.income * (r - l);
        tree_values[v].outcome += value.outcome * (r - l);
        return;
    }
    ...
    tree_values[v] =
            {(v * 2 < VERTEX_COUNT ? tree_values[v * 2].income : 0)
             ...
             (v * 2 < VERTEX_COUNT ? tree_values[v * 2].outcome : 0)
             + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1].outcome : 0)};
}

void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, double percentage) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return;
    }
    Push(v, l, r);
    if (ql <= l && r <= qr) {
        tree_factor[v] *= percentage;
        tree_add[v].income *= percentage;
        tree_values[v].income *= percentage;
        return;
    }
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            cout << ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to).Compute() << '\n';
