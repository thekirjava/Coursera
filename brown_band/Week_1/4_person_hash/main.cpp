...
struct AddressHasher {
    size_t operator()(const Address &a) const {
        return sHasher(a.city) * x * x + sHasher(a.street) * x + iHasher(a.building);
    }

    const size_t x = 14851;
    hash<string> sHasher;
    hash<int> iHasher;
};
...
struct PersonHasher {
    size_t operator()(const Person &p) const {
        return sHasher(p.name) * x * x * x + iHasher(p.height) * x * x + dHasher(p.weight) * x + aHasher(p.address);
    }

    const size_t x = 40591;
    hash<string> sHasher;
    hash<int> iHasher;
    hash<double> dHasher;
