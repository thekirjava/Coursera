...
namespace TransportDatabase {
    ...
    std::vector<Svg::SvgFigureHolder> StopNameLayer::createLayer(const TransportDatabase::Database &db) {
        ...
        underlayer.SetFillColor(config.render_params.underlayer_color);
        underlayer.SetStrokeColor(config.render_params.underlayer_color);
        underlayer.SetStrokeWidth(config.render_params.underlayer_width);
        ...
        for (const auto&[stop_name, stop_ptr] : sorted_stops) {
            Svg::Text name = text;
            Svg::Text underlayer_name = underlayer;
            name.SetData(stop_name);
            name.SetPoint(config.zoomIn(stop_ptr->getCoordinates()));
            underlayer_name.SetData(stop_name);
            underlayer_name.SetPoint(config.zoomIn(stop_ptr->getCoordinates()));
            result.push_back(Svg::MakeSvgFigure(underlayer_name));
    ...
    std::vector<Svg::SvgFigureHolder> BusNameLayer::createLayer(const Database &db) {
        config.flushColor();
        std::cerr << "Bus name layer" << std::endl;
        std::vector<Svg::SvgFigureHolder> result;
        std::map<std::string, std::shared_ptr<Bus>> sorted_buses;
        for (const auto&[bus_name, bus_ptr] : db.getBuses()) {
            sorted_buses[bus_name] = bus_ptr;
        }
        std::cerr << "Count of buses " << db.getStops().size() << std::endl;
        std::cerr << "Count of sorted buses " << db.getStops().size() << std::endl;
        Svg::Text base_text;
        base_text.SetOffset(config.render_params.bus_label_offset);
        base_text.SetFontSize(config.render_params.bus_label_font_size);
        base_text.SetFontFamily("Verdana");
        ...
        underlayer.SetStrokeLineJoin("round");
        for (const auto&[bus_name, bus_ptr] : sorted_buses) {
            Svg::Text stop = base_text;
            Svg::Text stop_underlayer = underlayer;
            stop.SetData(bus_name);
            stop_underlayer.SetData(bus_name);
            auto stop_names = bus_ptr->getStopsName();
            auto first_stop = db.getStop(*begin(stop_names));
            stop.SetPoint(config.zoomIn(first_stop->getCoordinates()));
            stop_underlayer.SetPoint(config.zoomIn(first_stop->getCoordinates()));
            stop.SetFillColor(config.getNewColor());
            result.push_back(Svg::MakeSvgFigure(stop_underlayer));
            result.push_back(Svg::MakeSvgFigure(stop));
            if ((bus_ptr->bus_type == Bus::BusTypes::LINEAR) && (*begin(stop_names) != *prev(end(stop_names)))) {
                Svg::Text stop2 = stop;
                Svg::Text stop_underlayer2 = stop_underlayer;
                auto last_stop = db.getStop(*prev(end(stop_names)));
                stop2.SetPoint(config.zoomIn(last_stop->getCoordinates()));
                stop_underlayer2.SetPoint(config.zoomIn(last_stop->getCoordinates()));
                result.push_back(Svg::MakeSvgFigure(stop_underlayer2));
                result.push_back(Svg::MakeSvgFigure(stop2));
            }
        }
        return result;
    }
