...
ComputeStopsCoords(Descriptions::StopsDict stops_dict, const Descriptions::BusesDict &buses_dict,
    ...
    vector<Descriptions::Bus> buses;
    buses.reserve(buses_dict.size());
    for (const auto&[_, bus_ptr]:buses_dict) {
        buses.push_back(*bus_ptr);
    }
    for (auto &stop:stops) {
        stop.supporting = Descriptions::isSupporting(stop, buses);
    }
    for (const auto &bus:buses) {
        Descriptions::Recount(stops, bus);
    }
    ...
    const Sphere::Projector projector(
            ...
            begin(buses), end(buses),
            max_width, max_height, padding
    );

    map<string, Svg::Point> stops_coords;
    for (const auto &stop: stops) {
        stops_coords[stop.name] = projector(stop.position);
        if (stop.supporting) {
            std::cerr << stop.name << '\n';
        }
    }
