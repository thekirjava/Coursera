...
namespace TransportDatabase {
    ...
    RenderParams DatabaseManager::extractRenderParams(const Json::Node &node) {
        ...
        rp.layers_order.reserve(layers.size());
        for (const auto& layer:layers) {
            if (layer.AsString() == "bus_lines") {
                rp.layers_order.push_back(LayersType::BUS);
            } else if (layer.AsString() == "bus_labels") {
                rp.layers_order.push_back(LayersType::BUS_NAME);
            } else if (layer.AsString() == "stop_points") {
                rp.layers_order.push_back(LayersType::STOP);
            } else if (layer.AsString() == "stop_labels") {
                rp.layers_order.push_back(LayersType::STOP_NAME);
            } else {
