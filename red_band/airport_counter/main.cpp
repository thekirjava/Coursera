...
public:
    ...
    AirportCounter(TIterator begin, TIterator end) {
        ...
        for (auto it = begin; it != end; it++) {
            items[static_cast<uint32_t>(*it)].second++;
        }
    }

    // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        return items[static_cast<uint32_t>(airport)].second;
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        items[static_cast<uint32_t>(airport)].second++;
    }
    ...
    void EraseAll(TAirport airport) {
        items[static_cast<uint32_t>(airport)].second = 0;
    }
...
private:
    ...
    Items items{make_pair(TAirport::Last_, 0)};
};
...
void TestManyConstructions() {
    LOG_DURATION("Too many constructions")
...
void TestMostPopularAirport() {
    LOG_DURATION("TestPopularAirport")
